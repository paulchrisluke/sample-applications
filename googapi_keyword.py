...
def goog_report_basic(client, report, date):
    ...
    report_query = (googleads.adwords.ReportQueryBuilder()
                    .Select('Date', 'Impressions', 'Clicks', 'Cost',
                            'AveragePosition', 'SearchExactMatchImpressionShare', 'SearchImpressionShare', 'SearchRankLostImpressionShare',
                            'Criteria', 'CreativeQualityScore', 'KeywordMatchType', 'QualityScore', 'PostClickQualityScore', 'IsNegative',
...
for account in my_account_id:
    ...
    for date in date_list:
        ...
        my_file_name = 'goog_' + account + '_' + 'keyword' + '_' + str(my_year) + str(my_month) + str(my_day) + '.csv'
