...
def goog_report_basic(client, report, date):
    ...
    report_query = (googleads.adwords.ReportQueryBuilder()
                    .Select('Date', 'Impressions', 'Clicks', 'Cost',
                            'AveragePosition', 'KeywordId', 'KeywordTextMatchingQuery', 'Query', 'QueryTargetingStatus', 'QueryMatchTypeWithVariant',
                            'Device', 'ExternalCustomerId', 'CampaignId', 'AdGroupId', 'CreativeId')
...
for account in my_account_id:
    ...
    for date in date_list:
        ...
        my_file_name = 'goog_' + account + '_' + 'searchquery' + '_' + str(my_year) + str(my_month) + str(my_day) + '.csv'
        my_file_pathname = report_pathname + '/' + my_file_name        
        digit = date.replace('-', '')
        adwords_date = str(digit) + ", " + str(digit)
        with open(my_file_pathname, 'w', encoding='utf-8') as adwords_report:
